...
class AssignmentList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        # Get assignments from the original project (where examples are stored)
        original_project = self.project.original_project or self.project
        queryset = self.model.objects.filter(project=original_project, assignee=self.request.user)
        return queryset

    def perform_create(self, serializer):
        # Create assignments in the original project (where examples are stored)
        original_project = self.project.original_project or self.project
...
class ResetAssignment(APIView):
    ...
    def delete(self, *args, **kwargs):
        # Delete assignments from the original project (where examples are stored)
        original_project = self.project.original_project or self.project
        Assignment.objects.filter(project=original_project).delete()
