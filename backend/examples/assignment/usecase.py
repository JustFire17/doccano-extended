...
def bulk_assign(project_id: int, strategy_name: StrategyName, member_ids: List[int], weights: List[int]) -> None:
    ...
    # Get unassigned examples from the original project
    unassigned_examples = Example.objects.filter(project=original_project, assignments__isnull=True)
    ...
    assignments = [
        Assignment(
            project=original_project,  # Create assignments in the original project
