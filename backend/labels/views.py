...
class BaseListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # Filter annotations by current project version to isolate annotations per version
        # This ensures that when you're in v2, you only see v2 annotations, not v1 annotations
        queryset = queryset.filter(project_version=self.project.version)
        
        if not self.project.collaborative_annotation and not self.request.user.is_superuser:
