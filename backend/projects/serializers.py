...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    discrepancy_active = serializers.BooleanField(required=False, default=False)
    perspective_associated = serializers.PrimaryKeyRelatedField(
        queryset=PerspectiveProject.objects.all(), required=False, allow_null=True
    )
    resourcetype = serializers.CharField(write_only=True, required=False)
    ...
    def create(self, validated_data):
        ...
        validated_data.pop("resourcetype", None)  # Remove write-only field
...
class DiscussionMessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "discussion", "sender", "sender_username", "content", "created_at"]
...
class DiscussionSerializer(serializers.ModelSerializer):
    ...
    def get_annotators(self, obj):
        ...
        project_members = Member.objects.filter(
            project=original_project, role__name__in=[settings.ROLE_ANNOTATOR, settings.ROLE_ANNOTATION_APPROVER]
        ).select_related("user", "role")

        annotators = []
        for member in project_members:
            if member.user:
                annotators.append({"id": member.user.id, "username": member.user.username, "role": member.role.name})
...
class DiscrepancyCommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "discrepancy", "user", "user_username", "content", "created_at"]
