...
from projects.serializers import (
    DiscrepancyCommentSerializer,
    ManualDiscrepancySerializer,
    PerspectiveMemberSerializer,
...
class ReopenProject(APIView):
    ...
    def post(self, request, project_id):
        ...
        return Response(
            {"message": "Project reopened successfully", "new_version": serializer.data}, status=status.HTTP_200_OK
        )
...
class CreateRuleWithItems(APIView):
    ...
    def post(self, request, project_id):
        ...
        for item in request.data.get("items", []):
            rule = Rule.objects.create(name=item.get("name"), description=item.get("description"), project=project)
            rules.append(rule)
...
class AssociatePerspectiveView(APIView):
    ...
    def post(self, request, project_id):
        ...
        if not perspective_project_id:
            return Response({"error": "perspective_project_id is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(
            {
                "message": (
                    f"Perspective '{perspective_project.name}' associated "
                    f"successfully with project '{project.name}'"
                )
            },
    ...
    def delete(self, request, project_id):
        ...
        return Response({"message": "Perspective association removed successfully"}, status=status.HTTP_200_OK)
    ...
    def post(self, request, project_id):
        ...
        for perspective_id, value in data.items():
            ...
            perspective = get_object_or_404(
                Perspective, id=perspective_id, perspective_project=project.perspective_associated
            )

            PerspectiveMember.objects.update_or_create(
                member=member, perspective=perspective, defaults={"value": value}
            )
...
class GetFilledPerspectives(APIView):
    ...
    def get(self, request, project_id):
        ...
        perspective_members = PerspectiveMember.objects.filter(
            member=member, perspective__perspective_project=project.perspective_associated
        )
...
class GetUsersWithValue(APIView):
    ...
    def get(self, request, project_id):
        ...
        if not perspective_id or not value:
            return Response({"error": "perspective_id and value are required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        perspective_members = PerspectiveMember.objects.filter(
            member__project_id__in=all_project_versions, perspective_id=perspective_id, value=value
...
class PerspectiveList(APIView):
    ...
    def delete(self, request, project_id, perspective_id):
        ...
        if not project.perspective_associated:
            return Response(
                {"error": "No perspective project associated with this project"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Check if user is the creator of the perspective
        if project.perspective_associated.created_by != request.user:
            return Response(
                {"error": "You don't have permission to delete these perspectives"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        return Response(
            {"message": f"Perspective '{perspective.name}' deleted successfully"}, status=status.HTTP_200_OK
        )
    ...
    def delete(self, request, project_id, perspective_id):
        ...
        if Project.objects.filter(perspective_associated=perspective_project).exists():
            return Response(
                {
                    "error": (
                        "Cannot delete this perspective project because it is "
                        "associated with one or more projects. Please dissociate "
                        "it from all projects first."
                    )
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {
                "message": (
                    f"Perspective '{perspective_project.name}' and all its items "
...
class DiscrepancyAnalysisView(APIView):
    ...
    def get(self, request, project_id):
        ...
        for example in examples:
            ...
            labels = (
                example.categories.filter(project_version=project.version)
                .values("label", "label__text")
...
class AnnotationsByUserView(APIView):
    ...
    def get(self, request, project_id):
        ...
        return Response({"items": items, "users": users}, content_type="application/json")
...
class LabelStatsView(APIView):
    ...
    def get(self, request, project_id, example_id):
        ...
        for a in annotations:
            ...
            result[label_text] = {"count": count, "percentage": percentage}
...
class AnnotatorReportView(APIView):
    ...
    def get(self, request, project_id):  # noqa: C901
        project = get_object_or_404(Project, id=project_id)
        ...
        annotations = (
            Category.objects.filter(example__project_id=original_project.id)
        ...
        if perspective_ids and perspective_values and project.perspective_associated:
            try:
                ...
                if perspective_id_list and perspective_value_list:
                    ...
                    perspective_members = (
                        PerspectiveMember.objects.filter(
        ...
        for annotation in annotations:
            ...
            if annotation.example.id not in unique_examples:
                example_text = (
                    annotation.example.text[:50] + "..."
                    if len(annotation.example.text) > 50
                    else annotation.example.text
                )
            ...
            result.append(
                {
                    ...
                    "example_text": annotation.example.text[:200] + "..."
                    if len(annotation.example.text) > 200
                    else annotation.example.text,
        ...
        return Response(
            {"annotations": result, "unique_examples": unique_examples_list, "project_versions": project_versions_list},
            status=status.HTTP_200_OK,
        )
...
class AnnotationStatisticsView(APIView):
    ...
    def get(self, request, project_id):
        ...
        annotation_ids = (
            request.query_params.get("exampleId", "").split(",") if request.query_params.get("exampleId") else []
        )
        perspective_ids = (
            request.query_params.get("perspectiveId", "").split(",")
            if request.query_params.get("perspectiveId")
            else []
        )
        perspective_values = (
            request.query_params.get("perspectiveValue", "").split(",")
            if request.query_params.get("perspectiveValue")
            else []
        )
        ...
        if perspective_ids and perspective_values:
            try:
                ...
                if perspective_ids and perspective_values:
                    perspective_members = PerspectiveMember.objects.filter(
                        perspective_id__in=perspective_ids, value__in=perspective_values, member__project=project
                    )
                    ...
                    if valid_members:
                        users_to_consider = (
                            PerspectiveMember.objects.filter(member_id__in=valid_members)
                            .values_list("member__user_id", flat=True)
                            .distinct()
                        )
        ...
        for example in examples:
            all_labels = (
                ...
                .values("label__id", "label__text", "label__background_color")
                .annotate(count=Count("label"))
                .distinct()
            )
            ...
            filtered_labels = (
                categories_query.values("label__id", "label__text", "label__background_color")
                .annotate(count=Count("label"))
                .distinct()
            )
            ...
            if total_filtered_labels > 0:
                ...
                users_result = list(categories_query.values("user__id", "user__username").distinct())
...
class AllVersionsStatisticsView(APIView):
    ...
    def get(self, request, project_id):
        ...
        if perspective_ids and perspective_values:
            perspective_members = PerspectiveMember.objects.filter(
                perspective_id__in=perspective_ids, value__in=perspective_values, member__project__in=versions
            )
        ...
        for version in versions:
            ...
            version_annotations = examples.filter(categories__project_version=version.version).distinct()

            for example in version_annotations:
                labels = (
                    example.categories.filter(project_version=version.version)
                    .values("label", "label__text")
                    .annotate(count=Count("label"))
                )
    ...
    def post(self, request, project_id, rule_id):
        try:
            ...
            if rule.voting_closed:
                return Response(
                    {"detail": "Voting for this rule is already closed."}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if existing_vote:
                ...
                # Create new vote
                RuleVote.objects.create(rule=rule, user=request.user, is_upvote=is_upvote)
...
class AnnotationLabelTableView(APIView):
    ...
    def get(self, request, project_id):
        ...
        try:
            ...
            all_labels = set()
            for example in examples:
                labels = example.categories.filter(project_version=project.version).values_list(
                    "label__text", flat=True
                )
            ...
            return Response({"labels": all_labels, "rows": table, "perspective_values": perspective_values_dict})
        except DatabaseError:
            return Response(
