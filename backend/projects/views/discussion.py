...
from projects.serializers import DiscussionMessageSerializer, DiscussionSerializer
...
class DiscussionList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return Discussion.objects.filter(project__in=all_project_versions).select_related("project", "created_by")
...
class DiscussionMessageList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return DiscussionMessage.objects.filter(
            discussion_id=self.kwargs["discussion_id"], discussion__project__in=all_project_versions
        )
    ...
    def perform_create(self, serializer):
        ...
        discussion = get_object_or_404(Discussion, id=self.kwargs["discussion_id"], project__in=all_project_versions)
        serializer.save(discussion=discussion, sender=self.request.user)
