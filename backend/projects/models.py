...
class Project(PolymorphicModel):
    ...
    perspective_associated = models.ForeignKey(
        "PerspectiveProject", on_delete=models.SET_NULL, null=True, blank=True, related_name="projects"
    )
    ...
    def create_new_version(self) -> "Project":
        ...
        max_version = Project.objects.filter(models.Q(id=original.id) | models.Q(original_project=original)).aggregate(
            models.Max("version")
        )["version__max"]
        ...
        for member in self.role_mappings.all():
            Member.objects.create(user=member.user, project=new_version, role=member.role)

        # Copy tags to new version
        for tag in self.tags.all():
            Tag.objects.create(text=tag.text, project=new_version)
                ...
                if new_member:
                    ...
                    for perspective in perspectives:
                        ...
                        current_value = PerspectiveMember.objects.filter(
                            member=current_member, perspective=perspective
                        ).first()

                        if current_value:
                            # Create the same value in the new version
                            PerspectiveMember.objects.create(
                                member=new_member, perspective=perspective, value=current_value.value
                            )
...
class PerspectiveProject(models.Model):
    ...
    updated_at = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="created_perspective_projects"
    )
...
class Perspective(models.Model):
    perspective_project = models.ForeignKey(
        PerspectiveProject, on_delete=models.CASCADE, related_name="perspectives", null=True
    )
...
class ManualDiscrepancy(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=[("unsolved", "Unsolved"), ("solved", "Solved")], default="unsolved"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
