    ...
    async def receive(self, text_data):
        try:
            ...
            if not message:
                ...
                await self.send(text_data=json.dumps({"error": "Falha ao salvar mensagem"}))
                return

            serializer = await self.get_message_data(message)

            # Send message to room group
            await self.channel_layer.group_send(self.room_group_name, {"type": "chat_message", "message": serializer})
    ...
    def save_message(self, user, content):
        try:
            ...
            if not user.is_authenticated:
                logger.error(
                    f"Usuário não autenticado tentando enviar mensagem para discussion_id={self.discussion_id}"
                )
                return None

            message = DiscussionMessage.objects.create(discussion=discussion, sender=user, content=content)
