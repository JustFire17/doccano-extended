...
for cmd in python3 poetry node npm; do
...
echo "[INFO] Ensuring backend dependencies are installed (poetry install)..."
(
    cd "$BACKEND"
    poetry install
)

echo "[INFO] Ensuring frontend dependencies are installed (npm install)..."
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    if ! grep -q "DEBUG.*True" "$ENV_FILE" || ! grep -q "^SECRET_KEY=" "$ENV_FILE"; then
        GENERATE_ENV=true
        echo "[INFO] .env exists but is incomplete for development, regenerating..."
    fi
fi
...
echo "   Frontend: http://localhost:3000 (or local network URL shown by Nuxt)"
