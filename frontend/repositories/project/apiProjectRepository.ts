...
export class APIProjectRepository {
  ...
  async list(query: SearchQuery): Promise<Page<Project>> {
    ...
    const projects = response.data.results.map((project: { [key: string]: any }) =>
      toModel(project)
    )
  ...
  async voteRule(projectId: number, ruleId: number, vote: boolean): Promise<Rule> {
    try {
      const response = await this.request.post(`/projects/${projectId}/rules/${ruleId}/vote/`, {
        is_upvote: vote
      })
      return Rule.create(
        response.data.id,
  ...
  async reopenRuleVote(
    ...
    try {
      const response = await this.request.post(
        `/projects/${projectId}/rules/${ruleId}/reopen-vote/`,
        {
          voting_end_date: votingEndDate || null,
          voting_end_time: votingEndTime || null
        }
