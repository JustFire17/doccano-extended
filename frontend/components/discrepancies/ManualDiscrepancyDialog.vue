<template>
  <v-dialog v-model="dialog" max-width="800px">
    <v-card>
      <v-card-title class="text-h6"> Report Discrepancy </v-card-title>
        ...
        <div v-if="labelStats.length">
          ...
          <v-list dense>
            <v-list-item v-for="(label, index) in labelStats" :key="index">
              <v-list-item-content>
                <v-chip :color="getColor(label.percentage)" class="ma-1" small>
                  {{ label.label_text }}
                </v-chip>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" :disabled="!isValid" :loading="isSubmitting" @click="submit">
          Report Discrepancy
        </v-btn>
      </v-card-actions>
    </v-card>
