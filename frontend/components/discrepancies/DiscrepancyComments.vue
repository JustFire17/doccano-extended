<template>
  <div>
    ...
    <form class="comment-form" @submit.prevent="addComment">
      <input v-model="newComment" class="comment-input" placeholder="Write a comment..." required />
...
import {
  ...
  postDiscrepancyComment
} from '@/repositories/discrepancy/apiDiscrepancyCommentRepository'
  ...
  setup(props) {
    ...
    const fetchComments = async () => {
      try {
        ...
        let message =
          "Sorry, we couldn't load the comments right now. Please try again in a few moments."
    ...
    const addComment = async () => {
      ...
      try {
        ...
        let message =
          "Sorry, we couldn't add your comment right now. Please try again in a few moments."
        if (error.response?.data?.detail) {
          message = error.response.data.detail
        }
        alert(message)
