<template>
  <v-card>
    <v-card-text>
      <v-text-field v-model="search" label="Search" single-line hide-details filled />
      <v-data-table
        :headers="headers"
        :items="items"
        :items-per-page="10"
      ...
      >
        ...
        <template #[`item.associated_projects`]="{ item }">
          <span
            v-if="!item.associated_projects || item.associated_projects.length === 0"
            style="color: #bdbdbd"
            >No associated projects</span
          >
          <span v-else>
        ...
        <template #expanded-item="{ headers: expandHeaders, item }">
          <td :colspan="expandHeaders.length">
            <v-data-table
              :headers="itemHeaders"
              :items="item.items || []"
              hide-default-footer
