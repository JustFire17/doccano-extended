<template>
  ...
  >
    ...
    <template #[`item.assignee`]="{ item }">
      ...
      >
        <template #selection="{ attrs, item: selectedMember, parent, selected }">
          <v-chip v-bind="attrs" :input-value="selected" small class="mt-1 mb-1">
            <span class="pr-1">{{ selectedMember.username }}</span>
            <v-icon small @click="parent.selectItem(selectedMember)"> $delete </v-icon>
          </v-chip>
        </template>
      </v-combobox>
    </template>
    <template #[`item.action`]="{ item }">
      <v-btn class="me-1" small color="primary text-capitalize" @click="$emit('edit', item)"
        >Edit</v-btn
      >
      <v-btn small color="primary text-capitalize" :disabled="projectClosed" @click="toLabeling(item)">
        {{ $t('dataset.annotate') }}
      </v-btn>
    </template>
  </v-data-table>
