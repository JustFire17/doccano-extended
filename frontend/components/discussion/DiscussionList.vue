<template>
  <v-card>
    <v-card-title>
      ...
      <v-spacer />
      <v-btn v-if="isProjectAdmin" color="primary" @click="handleCreateDiscussion">
        CREATE DISCUSSION
      </v-btn>
    </v-card-title>
    ...
    <v-card-text>
      ...
      <div v-if="versionFilter !== 'all'" class="mb-4">
        <v-alert color="info" outlined dense class="mb-3">
          <v-icon left>mdi-filter</v-icon>
          Showing discussions for
          {{ versionFilter === currentProject?.version ? 'current version' : 'version' }}
        ...
        <div v-if="currentVersionDiscussions.length > 0">
          ...
          >
            ...
            <template #[`item.projectVersion`]="{ item }">
              <v-chip small color="green" text-color="white"> v{{ item.projectVersion }} </v-chip>
            </template>
              ...
              <template v-if="isProjectAdmin">
                <v-tooltip bottom>
                  <template #activator="{ on, attrs }">
                    <v-btn icon small class="mr-2" v-bind="attrs" v-on="on" @click="openChat(item)">
                ...
                <v-tooltip bottom>
                  ...
                  <span>{{
                    item.status === 'closed'
                      ? 'Cannot Delete Closed Discussion'
                      : 'Delete Discussion'
                  }}</span>
                </v-tooltip>
              </template>
        ...
        <div v-if="historicalVersionDiscussions.length > 0">
          ...
          <div v-for="version in historicalVersions" :key="version" class="mb-4">
            ...
            >
              <template #[`item.name`]="{ item }">
                <router-link
                  :to="`/projects/${projectId}/discussions/${item.id}`"
                  class="grey--text"
                >
                  {{ item.name }}
                </router-link>
              ...
              <template #[`item.projectVersion`]="{ item }">
                <v-chip small outlined color="blue-grey"> v{{ item.projectVersion }} </v-chip>
              </template>
              ...
              <template #[`item.actions`]="{ item }">
                <template v-if="isProjectAdmin">
                  ...
                  <v-tooltip bottom>
                    <template #activator="{ on, attrs }">
                      <v-btn icon small color="error" v-bind="attrs" disabled v-on="on">
                        <v-icon>{{ mdiDeleteOutline }}</v-icon>
                      </v-btn>
                    </template>
      ...
      <div v-else class="text-center py-8">
        ...
        <h3 class="text-h6 grey--text mt-2">
          {{
            versionFilter !== 'all'
              ? `No discussions found for v${versionFilter}`
        ...
        <p class="grey--text">
          {{
            versionFilter !== 'all'
              ? 'Try selecting a different version or create a new discussion'
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-text-field v-model="newDiscussion.name" label="Discussion Name" required />
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="createDiscussion"> Create </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="red darken-1" text @click="deleteDiscussion"> Delete </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-card>
...
<script>
import { mdiForumOutline, mdiDeleteOutline } from '@mdi/js'
...
export default {
  ...
  computed: {
    ...
    versionOptions() {
      ...
      const allVersions = [...new Set(this.allDiscussions.map((d) => d.projectVersion))].sort(
        (a, b) => b - a
      )
      ...
      if (this.currentProject?.version) {
        options.push({
          text: `Current (v${this.currentProject.version})`,
          value: this.currentProject.version
        })
      }
    ...
    currentVersionDiscussions() {
      return this.filteredDiscussions.filter(
        (d) => d.projectVersion === this.currentProject?.version
      )
    },
    ...
    historicalVersions() {
      return [...new Set(this.historicalVersionDiscussions.map((d) => d.projectVersion))].sort(
        (a, b) => b - a
      )
    }
  },
  ...
  methods: {
    ...
    handleCreateDiscussion() {
      ...
      if (existingDiscussionForVersion) {
        alert(
          `Cannot create a new discussion. A discussion already exists for project version ${currentVersion}.`
        )
        return
      }
    ...
    async createDiscussion() {
      ...
      try {
        ...
        if (!currentUserId) {
          alert(
            `You must be logged in to create a discussion. Auth state: ${this.$store.state.auth.isAuthenticated}, Username: ${this.$store.state.auth.username}`
          )
          return
        }
    ...
    applyVersionFilter() {
      ...
      if (this.versionFilter === 'all') {
        ...
        this.discussions = this.allDiscussions.filter(
          (d) => d.projectVersion === this.versionFilter
        )
      }
    }
  }
