  ...
  <v-card>
    <v-toolbar dense flat color="primary" dark>
      ...
      <v-toolbar-title>
        ...
        <v-chip v-if="discussion" :color="statusColor" small class="ml-2">
          {{ discussion.status }}
        </v-chip>
      </v-toolbar-title>
      ...
      <div v-if="discussion" class="text-caption mr-4">
        <div>
          <small>Created: {{ formatDate(discussion.createdAt) }}</small>
        </div>
              ...
              >
                <v-card-text>
                  <div class="d-flex align-center">
                    <v-avatar size="28" :color="getUserColor(message.senderUsername)" class="mr-2">
                      <span class="white--text">{{ getUserInitial(message.senderUsername) }}</span>
                    </v-avatar>
                    <strong>{{ message.senderUsername }}</strong>
                    <small
                      class="ml-2 grey--text"
                      :class="{ 'text--lighten-3': isSentByCurrentUser(message) }"
                    >
  ...
  methods: {
    ...
    async fetchDiscussion() {
      ...
      try {
        ...
        this.discussion = await this.$services.discussion.getDiscussion(
          this.projectId,
          this.discussionId
        )
    ...
    async fetchMessages() {
      try {
        ...
        this.messages = await this.$services.discussion.getMessages(
          this.projectId,
          this.discussionId
        )
        this.scrollToBottom()
    ...
    sendMessageWebSocket(message) {
      if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
        ...
        // Fallback to REST API
        this.$services.discussion
          .createMessage(this.projectId, this.discussionId, message)
