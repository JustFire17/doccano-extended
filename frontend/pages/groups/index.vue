<template>
  <v-container class="pt-16">
    <!-- Botão de Criação -->
    <v-btn
      class="text-capitalize mt-4 d-flex justify-end"
      color="primary"
    ...
    <v-data-table
      ...
      :no-data-text="$t('There is no groups listed at the moment')"
      :footer-props="{
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="createForm">
            ...
            <v-combobox
              v-model="createForm.selectedPermissions"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="editForm">
            ...
            <v-combobox
              v-model="editForm.selectedPermissions"
              :items="allPermissions"
              item-text="description"
              item-value="id"
              label="Permissions"
  ...
  methods: {
    async fetchGroups() {
      try {
        ...
        let message =
          "Sorry, we couldn't load the groups right now. Please try again in a few moments."
    ...
    openEditDialog(group) {
      this.editForm = {
        ...
        selectedPermissions: group.permissions.map((permissionId) => {
          return (
            this.allPermissions.find((permission) => permission.id === permissionId) || {
              id: permissionId
            }
          )
        })
    ...
    async createGroup() {
      try {
        ...
        let message =
          "Sorry, we couldn't create the group right now. Please try again in a few moments."
        if (error.response?.data?.detail) {
          message = error.response.data.detail
        }
        alert(message)
    ...
    async editGroup() {
      try {
        ...
        let message =
          "Sorry, we couldn't update the group right now. Please try again in a few moments."
        if (error.response?.data?.detail) {
          message = error.response.data.detail
        }
        alert(message)
    ...
    async deleteGroup(groupId) {
      if (confirm('Are you sure you want to delete this Group?')) {
        try {
          ...
          let message =
            "Sorry, we couldn't delete the group right now. Please try again in a few moments."
          if (error.response?.data?.detail) {
            message = error.response.data.detail
          }
          alert(message)
