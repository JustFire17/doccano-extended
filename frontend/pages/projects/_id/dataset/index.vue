  ...
  <v-card>
    <v-card-title v-if="isProjectAdmin">
      ...
      <v-btn
        v-if="isProjectAdmin"
        class="text-capitalize ms-2"
        :color="project.closed ? 'success' : 'warning'"
        @click="project.closed ? reopenProject() : closeProject()"
      >
        {{ project.closed ? 'Reopen Project' : 'Close Project' }}
      </v-btn>
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      labelCounts: {} as Record<number, number>,
      labelPercentages: {} as Record<number, number>,
    }
  },
  ...
  methods: {
    ...
    async closeProject() {
      try {
        await this.$services.project.closeProject(this.projectId)
    ...
    async reopenProject() {
      try {
        const newProject = await this.$services.project.reopenProject(this.projectId)
        // Update the project in the store with the new version
        this.$store.commit('projects/setProject', newProject)
