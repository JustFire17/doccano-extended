...
export default {
  ...
  methods: {
    ...
    async canConfirm() {
      ...
      const filledValues = await this.$services.perspective.getFilledValues(this.projectId)
      ...
      if (!allFilled) {
        ...
        this.$router.push(`/projects/${this.projectId}/perspective/fill`)
        return false
      }
    ...
    async confirm() {
      const allowedToConfirm = await this.canConfirm()
      if (!allowedToConfirm) {
        return
      }
