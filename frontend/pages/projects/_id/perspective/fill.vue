  ...
  <v-card>
    ...
    <v-card-text>
      <v-alert v-if="!hasPerspective" type="warning">
        This project doesn't have an associated perspective set. Please ask an administrator to
        associate a perspective with this project.
      </v-alert>
    ...
    <v-card-actions class="ps-4 pt-0">
      <v-btn
        v-if="!isEditing && hasPerspective"
        color="primary"
        class="text-capitalize"
        @click="checkAnnotationsAndEdit"
      >
        Edit
      </v-btn>
      <v-btn
  ...
  computed: {
    ...
    processedPerspectives(): any[] {
      return this.items.map((perspective) => ({
        ...perspective,
        optionsArray: perspective.options
          ? perspective.options.split(';').map((opt) => opt.trim())
          : []
      }))
    }
  },
        ...
        if (examples.items && examples.items.length > 0) {
          ...
          for (const example of examples.items) {
            ...
            if (annotations && annotations.length > 0) {
              alert(
                'You cannot edit perspectives after making annotations. Please delete your annotations first.'
              )
