            ...
            <template #item="{ item }">
              <tr>
                ...
                <td v-for="value in selectedValues" :key="value">
                  <div
                    v-if="item.statistics[value] && item.statistics[value].length > 0"
                    class="label-stats"
                  >
                  ...
                  <div v-else class="text-center grey--text">No annotations</div>
                </td>
              </tr>
            </template>
            <template #no-data>
              <div class="text-center pa-4">
                {{
                  loadingSearch
                    ...
                    ? 'No annotations'
                    : 'No data available. Select filters and click Search.'
                }}
              </div>
            </template>
          </v-data-table>
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      sortBy: ['example'],
      sortDesc: [false],
      headers: [{ text: 'Example', value: 'example', sortable: true, align: 'start' }]
    }
  },

  async fetch() {
    await Promise.all([this.loadExamples(), this.loadPerspectives()])
  },
  ...
  computed: {
    ...
    tableHeaders() {
      const baseHeaders = [{ text: 'Example', value: 'example', sortable: true, align: 'start' }]
    ...
    async searchAnnotations() {
      ...
      try {
        ...
        for (const example of examples) {
          ...
          const allPerspectiveValues = await this.$services.perspective.getAllFilledValues(
            this.projectId
          )
