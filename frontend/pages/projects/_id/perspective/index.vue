  ...
  <div>
    <v-card>
      <v-card-title>
        ...
        <v-btn class="text-capitalize ms-2" color="primary" @click="openAssociationDialog">
          {{ hasAssociatedPerspective ? 'Dissociate Perspective' : 'Associate Perspective' }}
        </v-btn>

        <v-btn class="text-capitalize ms-2" color="error" @click="openDeleteDialog">
          Delete Perspective
        </v-btn>
      </v-card-title>
      ...
      <v-card>
        ...
        <v-card-text>
          <div
            v-if="
              !selectedPerspective ||
              !selectedPerspective.associated_projects ||
              selectedPerspective.associated_projects.length === 0
            "
            style="color: #bdbdbd"
          >
          ...
          <v-list v-else>
            <v-list-item
              v-for="project in selectedPerspective.associated_projects"
              :key="project.id"
            >
              <v-list-item-content>
  ...
  methods: {
    async loadData() {
      ...
      try {
        ...
        this.error =
          ...
          "Sorry, we couldn't load the perspectives right now. Please try again in a few moments."
        alert(this.error)
      } finally {
    ...
    async handleDissociate() {
      try {
        ...
        alert(
          error.response?.data?.error || 'Error when dissociating perspective. Please try again.'
        )
      }
    },
