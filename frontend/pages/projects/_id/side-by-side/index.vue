<template>
  <v-container>
    <v-card>
      <v-card-title class="text-h5"> Side-by-Side Annotations </v-card-title>
      <v-card-text>
        ...
        <v-row>
          <v-col cols="12" md="6">
            ...
            >
              <template #selection="{ item, index }">
                <v-chip v-if="index < 3" :key="item" close @click:close="removeUser(item)">
                  {{ item }}
                </v-chip>
          ...
          <v-col cols="12" md="6">
            ...
            >
              <template #selection="{ item, index }">
                <v-chip v-if="index < 3" :key="item.id" close @click:close="removeExample(item)">
...
export default Vue.extend<ComponentData, ComponentMethods, ComponentComputed>({
  ...
  async fetch() {
    await Promise.all([this.loadExamples(), this.loadUsers(), this.loadCategoryTypes()])
  },
  computed: {
    projectId() {
    ...
    tableHeaders() {
      const baseHeaders = [{ text: 'Example', value: 'text', sortable: true, align: 'start' }]
