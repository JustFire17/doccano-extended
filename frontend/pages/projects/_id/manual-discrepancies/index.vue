<template>
  <v-container class="pt-10" fluid>
    <v-card>
      <v-card-title class="text-h5"> Discrepancies </v-card-title>
      <v-card-text>
        <v-data-table
          :headers="headers"
          :items="items"
          ...
          <template #[`item.example_text`]="{ item }">
            <div class="dataset-text-container">
              <div style="white-space: pre-wrap">
                {{ item.example_text || `Loading example #${item.example}...` }}
              </div>
            </div>
          </template>
        ...
        <v-dialog v-model="showCommentsModal" max-width="600px">
          <v-card>
            ...
            <v-card-text>
              <DiscrepancyComments
                v-if="selectedDiscrepancyId"
                :discrepancy-id="selectedDiscrepancyId"
              />
            </v-card-text>
          </v-card>
        </v-dialog>
...
export default Vue.extend({
  ...
  async fetch() {
    ...
    try {
      ...
      this.items = discrepancies.map(
        (item: ManualDiscrepancy): TableItem => ({
          ...item,
