        ...
        <v-col cols="12" md="9">
          ...
          >
            ...
            <template #[`item.version`]="{ item }">
              <v-chip v-if="item.version" color="blue" text-color="white" small>
                v{{ item.version }}
              </v-chip>
            ...
            <template #[`item.actions`]="{ item }">
              <v-btn color="primary" text small @click="viewAnnotation(item)">
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12">
                      <v-textarea label="Text" :value="selectedAnnotation.text" readonly rows="4" />
                    </v-col>
                    <v-col cols="12">
                      <v-text-field
                        label="Created At"
                    ...
                    <v-col cols="12">
                      ...
                      <div
                        v-if="selectedAnnotation.users && selectedAnnotation.users.length > 0"
...
export default {
  ...
  methods: {
    ...
    async fetchAnnotations() {
      ...
      try {
        const filters = {
          ...
          perspectiveIds:
            this.filters.perspectiveId.length > 0 ? this.filters.perspectiveId : undefined,
          perspectiveValues:
            this.selectedPerspectiveValues.length > 0 ? this.selectedPerspectiveValues : undefined,
          annotationIds: this.filters.exampleId.length > 0 ? this.filters.exampleId : undefined,
          projectVersion:
            this.filters.projectVersion.length > 0 ? this.filters.projectVersion : undefined
        }
    ...
    populateFilterOptions(annotations) {
      ...
      this.availableLabels = Array.from(labels.values()).sort((a, b) =>
      ...
      const versions = [
        ...new Set(
          annotations.map((item) => item.version).filter((v) => v !== null && v !== undefined)
        )
      ]
