  ...
  <v-card>
    ...
    <v-card-title>
      ...
      <v-btn
        ...
        :disabled="!canDelete || project.closed"
        outlined
    ...
    <label-list
      ...
      :disable-edit="canOnlyAdd || project.closed"
      @edit="editItem"
    />
  </v-card>
</template>
  ...
  methods: {
    ...
    async remove() {
      if (this.project.closed) {
        alert('Não é possível remover labels em um projeto fechado.')
        return
      }
    ...
    editItem(item: LabelDTO) {
      if (this.project.closed) {
        alert('Não é possível editar labels em um projeto fechado.')
        return
      }
