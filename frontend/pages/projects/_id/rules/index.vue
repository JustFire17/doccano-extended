  ...
  <v-container>
            ...
            >
              ...
              <template #[`item.votingEnd`]="{ item }">
                <div>
                  {{
                    item.votingEndDate
                      ? formatDate(item.votingEndDate + ' ' + (item.votingEndTime || '23:59'))
                      : '-'
                  }}
                </div>
              </template>
              ...
              <template #[`item.vote`]="{ item }">
                <div v-if="isProjectAdmin">
                  ...
                  <span v-else class="text-caption grey--text"> Can't edit previous version </span>
                </div>
                <div v-else>
                  <v-btn
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="form" v-model="valid">
            ...
            <v-textarea v-model="newRule.description" label="Description" required></v-textarea>
            <v-menu
              ref="dateMenu"
              v-model="dateMenu"
    ...
    <v-dialog v-model="voteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text> Are you sure you want to submit your vote? </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="voteDialog = false"> Cancel </v-btn>
    ...
    <v-dialog v-model="closeVoteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text> Are you sure you want to close this vote? </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="closeVoteDialog = false"> Cancel </v-btn>
          <v-btn color="primary" @click="executeCloseVote"> Confirm </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="reopenVoteDialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="reopenVoteDialog = false"> Cancel </v-btn>
          <v-btn color="primary" @click="executeReopenVote"> Confirm </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="deleteRules"> Confirm </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default Vue.extend({
  ...
  methods: {
    resetNewRule() {
      ...
      if (form && typeof form.resetValidation === 'function') {
    ...
    async voteRule(rule, isUpvote) {
      try {
        ...
        if (error.response?.status === 500) {
          this.errorMessage =
            error.response.data?.detail || 'Database unavailable. Please try again later.'
