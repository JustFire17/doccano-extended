  ...
  <div>
    <v-card>
      <v-card-title>
        ...
        <v-spacer></v-spacer>
        <v-btn color="primary" :loading="loading" @click="applyFilters"> Show Statistics </v-btn>
        <v-btn color="primary" type="button" @click="exportBoth"> Export </v-btn>
      </v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12" md="4">
...
export default Vue.extend({
  ...
  computed: {
    ...
    tableHeaders(): any[] {
      const base = [{ text: 'Annotation Text', value: 'text', align: 'start', sortable: false }]
      base.push(
  ...
  methods: {
    ...
    applyFilters() {
      ...
      if (this.selectedLabels.length > 0) {
        filtered = filtered.filter((row) =>
          this.selectedLabels.some((label) => row.labels[label] > 0)
        )
      }
    ...
    exportToCSV() {
      ...
      link.setAttribute(
        'download',
        `label_table_per_annotation_${new Date().toISOString().split('T')[0]}.csv`
      )
