<template>
  <div>
    <v-card>
      <v-card-title>
        <span class="headline">Annotation Statistics</span>
        <v-spacer></v-spacer>
        <v-btn color="primary" :loading="loading" @click="showStatistics"> Show Statistics </v-btn>
      ...
      <v-card-text>
        ...
        <v-row v-if="!loading && showResults">
          <v-col
            v-for="(annotation, annotationIndex) in annotationResults"
            :key="annotationIndex"
            cols="12"
          >
            <v-card class="mb-2 pa-2">
              <v-row align="start">
                <!-- Texto da anotação -->
                <v-col cols="12" md="6">
                  <div class="subtitle-1 font-weight-bold mb-1">Annotation Text</div>
                  <div>{{ annotation.text }}</div>
                </v-col>
                <!-- Discrepância e labels -->
                <v-col cols="12" md="6" class="d-flex flex-column justify-start">
                  <div class="mb-2">
                    ...
                    <v-chip
                      v-if="annotation.discrepancy"
                      :color="annotation.discrepancy_status === 'Reported' ? 'success' : 'warning'"
                      class="ml-2"
                    >
...
interface StatisticsAnnotation {
  id: number
  text: string
  labels: Label[]
  discrepancy?: boolean
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      annotationResults: [] as StatisticsResponse['annotations'],
  ...
  async created() {
    await Promise.all([this.loadAnnotations(), this.loadPerspectives(), this.loadProject()])
  },
  ...
  methods: {
    ...
    async onPerspectiveChange() {
      ...
      if (this.filters.perspectiveId && this.filters.perspectiveId.length > 0) {
        try {
          const allValues = (await this.$services.perspective.getAllFilledValues(
            this.projectId
          )) as PerspectiveValues
      ...
      try {
        ...
        if (this.filters.annotation.length > 0) {
          this.filters.annotation.forEach((id) => {
            params.append('exampleId', id.toString())
          })
        }

        if (this.filters.perspectiveId && this.selectedPerspectiveValues) {
          this.filters.perspectiveId.forEach((perspectiveId) => {
            params.append('perspectiveId', perspectiveId.toString())
          })

          this.selectedPerspectiveValues.forEach((value) => {
            params.append('perspectiveValue', value)
          })
        }

        const response = (await this.$services.project.getAnnotationStatistics(
          this.projectId,
          params.toString()
        )) as StatisticsResponse

        this.annotationResults = response.annotations
    ...
    async exportToPDF() {
      try {
        ...
        let filtersHtml = ''
        if (this.filters.annotation.length > 0) {
          const selectedAnnotations = this.annotations.filter((a) =>
            this.filters.annotation.includes(a.id)
          )
          filtersHtml += `<div><strong>Filtered Annotations:</strong> ${selectedAnnotations
            .map((a) => a.text)
            .join(', ')}</div>`
        }
        if (this.filters.perspectiveId.length > 0) {
          const selectedPerspectives = this.perspectives.filter((p) =>
            this.filters.perspectiveId.includes(p.id)
          )
          filtersHtml += `<div><strong>Filtered Perspectives:</strong> ${selectedPerspectives
            .map((p) => p.name)
            .join(', ')}</div>`
        }
        if (this.selectedPerspectiveValues.length > 0) {
          filtersHtml += `<div><strong>Perspective Values:</strong> ${this.selectedPerspectiveValues.join(
            ', '
          )}</div>`
        }
          ...
          <div style='margin-top: 20px;'>
            ${this.annotationResults
              ...
              <div style='margin-bottom: 50px; page-break-inside: avoid;'>
                ...
                <p><strong>Annotation Text:</strong> ${annotation.text}</p>
                ${
                  annotation.discrepancy
                    ? `<div style='margin-bottom: 8px;'><span style='color: #e53935; font-weight: bold;'>Discrepancy</span> <span style='color: ${
                        annotation.discrepancy_status === 'Reported' ? '#43a047' : '#ffa000'
                      }; font-weight: bold;'>${annotation.discrepancy_status}</span></div>`
                    : ''
                }
                <div style='margin-top: 10px;'>
                  ...
                  <div style='display: flex; align-items: center; gap: 10px; margin-bottom: 5px;'>
                    ${annotation.labels
                      ...
                      <span style='font-weight: bold; margin-right: 10px;'>${
                        label.text
                      } (${Math.round(label.percentage)}%)</span>
                    `
                      )
                      .join('')}
                  </div>
                  <div style='display: flex; width: 100%; height: 30px; border-radius: 4px; overflow: hidden;'>
                    ${annotation.labels
                      .map(
                        (label) => `
                      <div style='width: ${label.percentage}%; background-color: ${
                          label.backgroundColor
        ...
        const opt = {
          margin: 1,
          filename: `annotation_statistics_${this.currentProject.name}_${
    ...
    exportToCSV() {
      // Cabeçalho
      const header = ['Annotation Text', 'Labels', 'Discrepancy', 'Discrepancy Status']
      // Linhas
      const rows = this.annotationResults.map((annotation) => {
        const labels = annotation.labels
          .map((l) => `${l.text} (${Math.round(l.percentage)}%)`)
          .join(', ')
        return [
      ...
      link.setAttribute(
        'download',
        `annotation_statistics_${this.currentProject.name}_${
          new Date().toISOString().split('T')[0]
        }.csv`
      )
