            ...
            <v-card-text>
              <v-form ref="form">
                ...
                >
                  ...
                  <v-date-picker v-model="filters.dateStart" @input="updateStartDate">
                    <v-spacer></v-spacer>
                ...
                >
                  <template #activator="{ on, attrs }">
                    ...
                    >
                      <template #append>
                        <v-btn v-if="endDateText !== 'Present'" icon small @click="clearEndDate">
                          <v-icon>mdi-close</v-icon>
                        </v-btn>
                      </template>
                    </v-text-field>
                  </template>
                  <v-date-picker v-model="filters.dateEnd" @input="updateEndDate">
                    <v-spacer></v-spacer>
                ...
                <v-card-text>
                  ...
                  >
                    ...
                    <template #[`item.label_text`]="{ item }">
                      <v-chip :color="item.backgroundColor || 'grey'" text-color="white" small>
                        {{ item.label_text }}
                      </v-chip>
                    </template>
                  </v-data-table>
    ...
    <v-dialog v-model="exportDialog" max-width="400px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" text :disabled="!isExportOptionSelected" @click="exportData"
            >Export</v-btn
          >
        </v-card-actions>
      </v-card>
...
>({
  ...
  computed: {
    ...
    stats() {
      ...
      const annotators = [
        ...new Set(this.annotationData.map((item: AnnotationItem) => item.username))
      ]
  ...
  async mounted() {
    try {
      ...
      this.startDateText = this.filters.dateStart
        ? this.formatDate(this.filters.dateStart)
        : 'All time'
  ...
  methods: {
    ...
    async fetchAnnotations() {
      ...
      try {
        ...
        try {
          const response: any = await this.$services.report.getAnnotatorReports(
            this.projectId,
            filters
          )
      ...
      if (this.filters.perspectiveId && this.filters.perspectiveId.length > 0) {
        try {
          const allValues: Record<string, string[]> =
            await this.$services.perspective.getAllFilledValues(this.projectId)
    ...
    populateExampleOptions(data: AnnotationItem[]) {
      ...
      this.exampleOptions = Array.from(examples.values()).sort((a, b) =>
    ...
    async exportToPDF(): Promise<void> {
      try {
        ...
        if (this.filters.perspectiveId && this.filters.perspectiveId.length > 0) {
          const perspectiveNames = this.filters.perspectiveId
            .map((id) => {
        ...
        footer.textContent = `Exported on ${new Date().toLocaleString()} | Total records: ${
          this.annotationData.length
    ...
    async exportToCSV(): Promise<void> {
      try {
        ...
        a.setAttribute(
          'download',
          `annotator-report-detailed-${new Date().toISOString().slice(0, 10)}.csv`
        )
