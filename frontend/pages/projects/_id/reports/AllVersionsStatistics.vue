          ...
          <v-col v-for="version in annotationResults" :key="version.version_id" cols="12">
            <v-card class="mb-4">
              <v-card-title>
                ...
                <v-chip v-if="version.is_current_version" color="primary" small class="ml-2">
                  Current
                </v-chip>
              </v-card-title>
              <v-card-text>
                <v-row
                  v-for="(annotation, annotationIndex) in version.annotations"
                  :key="annotationIndex"
                >
                  <v-col cols="12">
                          ...
                          <v-col cols="12" md="6" class="d-flex flex-column justify-start">
                            <div class="mb-2">
                              ...
                              <v-chip
                                ...
                                :color="
                                  annotation.discrepancy_status === 'Reported'
                                    ? 'success'
                                    : 'warning'
                                "
                                class="ml-2"
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        let filtersHtml = ''
        if (this.filters.versions.length > 0) {
          const selectedVersions = this.projectVersions.filter((v) =>
            this.filters.versions.includes(v.id)
          )
          filtersHtml += `<div><strong>Filtered Versions:</strong> ${selectedVersions
            .map((v) => v.displayVersion)
            .join(', ')}</div>`
        }
        if (this.filters.annotation.length > 0) {
          ...
          <div style='margin-top: 20px;'>
            ${this.annotationResults
              ...
              <div style='margin-bottom: 50px; page-break-inside: avoid;'>
                <h3>Version ${version.version_number}${
                  version.is_current_version ? ' (Current)' : ''
                }</h3>
                ${version.annotations
                  .map(
                    (annotation, index) => `
        ...
        const opt = {
          margin: 1,
          filename: `all_versions_statistics_${this.currentProject.name}_${
    ...
    exportToCSV() {
      ...
      link.setAttribute(
        'download',
        `all_versions_statistics_${this.currentProject.name}_${
          new Date().toISOString().split('T')[0]
        }.csv`
      )
